...
// nil (same shape as the dns/mail plugin row helpers).
...
// splitPorts returns the comma-separated CleanPorts tokens as a slice.
// Empty cleaned input yields a nil/empty slice (not [""]).
func splitPorts(cleaned string) []string {
	if cleaned == "" {
		return nil
	}
...
// portSet builds a set from a port token list for membership tests.
