...
- [ ] 1.1 `Vagrantfile`: add `legacy-apache` (bento/ubuntu-24.04, `192.168.56.21`, 2 GB) and raise `legacy` resources for the full stack; both marked standing-lab (excluded from `vagrant-destroy`); document the IP table in the file header. Commit.
...
- [ ] 3.1 WordPress: install 4 total (2 per VM) into fixture sites using the fixture DBs/FTP users — wp-cli unattended install; verify front page HTTP 200 with rendered content and wp-admin login; screenshots. Commit.
