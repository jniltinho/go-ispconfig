...
- [ ] 3.1 Implement plugin connection lifecycle and denylists (`root`, `debian-sys-maint`, `mysql.infoschema`; `mysql`, `information_schema`, `performance_schema`); skip non-`mysql` types; tests. Commit.
- [ ] 3.2 Implement `getHostList` / `getOtherHostList` (localhost always; remote_access + remote_ips → IPs or `%`; union of other active DBs for the user); table-driven unit tests. Commit.
...
- [ ] 3.7 Implement `renameDatabase` (empty path + tables/views/triggers path with mysqldump temp files mode 0600); integration test rename with a base table. Commit.
...
- [ ] 5.1 Database endpoints under `/api/sites/databases` (list/get/create/update/delete) via declarative Entity + swaggo annotations; handler tests incl. 403 cross-client. Commit.
