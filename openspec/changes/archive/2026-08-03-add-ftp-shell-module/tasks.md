...
- [x] 7.3 Note in `add-installer-cli` / installer tracking (or ROADMAP cross-link) that `configure_pureftpd` / `configure_jailkit` / `pureftpd_mysql.conf.master` remain a Modified Capability of the installer change when this module is scheduled — no installer code in this change unless that change is co-scheduled. Commit.
