...
- [ ] 1.1 Add GORM models for `client_template`, `client_template_assigned`, `client_message_template` and `country` with explicit `gorm:"column:..."` tags matching `ispconfig3.sql`; confirm existing `Client` / `SysUser` / `SysGroup` models cover every column used by this change; unit-test round-trip against MariaDB. Commit.
- [ ] 1.2 Implement client domain helpers: reseller detection (`limit_client != 0`), parent validation (one-level nesting), username uniqueness against `client` + `sys_user`, password hashing via `auth.HashPassword` for new passwords. Table-driven tests. Commit.
