...
- [ ] 2.1 Implement `internal/cron` Module: announce `cron_insert` / `cron_update` / `cron_delete`, register table hook for `cron`, map datalog actions `i`/`u`/`d` to events; gate on `server.web_server=1` + config.toml enablement; unit tests with fake registries. Commit.
