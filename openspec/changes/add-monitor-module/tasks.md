...
- [ ] 4.1 Add monitor routes: `GET /api/monitor/state`, `GET /api/monitor/data`, `GET /api/monitor/data/{type}` with module gate + server scope; handler tests. Commit.
- [ ] 4.2 Add `GET /api/monitor/sys-log` and admin `POST /api/monitor/sys-log/clear`; tests for 403 non-admin and loglevel-zero semantics. Commit.
