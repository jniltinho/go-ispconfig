...
- [x] 4.4 Extend `GET /api/system/scheduler` DTO with `spec` and `next_run` (admin-only preserved); compute next_run from stored spec; tests. Commit.
- [x] 4.5 Swaggo annotations for all new/changed endpoints; run `make swagger`; verify Swagger UI lists Monitor + updated scheduler schemas; CI staleness check green. Commit.
