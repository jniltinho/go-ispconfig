...
const errorList = () =>
  Object.entries(props.errors ?? {}).filter(([, msgs]) => msgs.length > 0);
  ...
  (errors) => {
    ...
    const hasBadField = (name: string) =>
      props.metadata.tabs
        ...
        ?.fields.some((f) => bad.has(f.name));
    if (hasBadField(activeTab.value)) return;
    const offending = props.metadata.tabs.find((tab) =>
      tab.fields.some((f) => bad.has(f.name)),
    );
  ...
  >
    ...
    >
      ...
      <ul class="list-disc py-2 pl-8 pr-3">
        <li v-for="[field, msgs] in errorList()" :key="field">
          {{ msgs.join(", ") }}
        </li>
      </ul>
    </div>
      ...
      <template v-for="tab in metadata.tabs" :key="tab.name">
        <div v-show="activeTab === tab.name" class="space-y-4">
          <template v-for="field in tab.fields" :key="field.name">
            <!-- Fieldset legend rendered as a sub-heading (original
                 fieldset-legend trait) -->
            <p
